...
async def admin_list_features(suburb: str = "", state: str = "", postcode: str = "",
    ...
    if postcode:
        ...
        rows = conn.execute("SELECT * FROM features ORDER BY confidence DESC LIMIT 500").fetchall()
    conn.close()
    return {"features": [dict(zip([d[0] for d in rows[0].keys()] if rows else [], r)) if hasattr(r, 'keys') else dict(r) for r in rows]}
