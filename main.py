...
def gen_features(lat, lng, suburb, state, postcode, count=50):
    """Generate demo/preview features. GPS + dimensions only — no fake addresses or DA refs."""
...
def get_real_features(suburb, state, postcode=None):
    ...
    if not rows:
        return None
    cols = rows[0].keys()
    return [{col: row[col] for col in cols} for row in rows]
...
async def admin_upload_db(request: Request, ps_session: str = Cookie(None)):
    ...
    try:
        ...
        # Detect table name — Colab might use 'features' or 'detections'
        tables = [t[0] for t in src.execute("SELECT name FROM sqlite_master WHERE type='table'").fetchall()]
        table = "features" if "features" in tables else tables[0] if tables else None
        if not table:
            src.close(); tmp.unlink(missing_ok=True)
            raise HTTPException(400, "No tables found in database")
        rows = src.execute(f"SELECT * FROM {table}").fetchall()
        cols = [desc[0] for desc in src.execute(f"SELECT * FROM {table} LIMIT 1").description] if rows else []
        src.close()
        conn = get_db(); imported = 0
        for row in rows:
            r = dict(zip(cols, row))  # Convert to plain dict
            fid = r.get("id", hashlib.md5(f"{r.get('lat',0)}{r.get('lng',0)}".encode()).hexdigest()[:12])
            conn.execute("""INSERT OR REPLACE INTO features
                ...
                VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
                (fid, r.get("lat",0), r.get("lng",0), r.get("address",""),
                 r.get("suburb",""), r.get("state",""), r.get("postcode",""),
                 r.get("property_type","Residential"), r.get("length_m",0), r.get("width_m",0),
                 r.get("area_m2",0), r.get("variant",""), r.get("finish","Unknown"),
                 r.get("est_capacity",0), r.get("confidence",0), r.get("ref_number",""),
                 r.get("year_installed",0), r.get("scan_id","")))
            imported += 1
        conn.commit(); conn.close(); tmp.unlink(missing_ok=True)
        return {"ok": True, "imported": imported, "table": table, "columns": cols}
...
@app.delete("/admin/features/{feature_id}")
async def admin_delete_feature(feature_id: str, ps_session: str = Cookie(None)):
    if not check_admin(ps_session): raise HTTPException(401)
    conn = get_db()
    conn.execute("DELETE FROM features WHERE id=?", (feature_id,))
    conn.commit(); conn.close()
    return {"ok": True, "deleted": feature_id}

@app.post("/admin/features/bulk-delete")
async def admin_bulk_delete(request: Request, ps_session: str = Cookie(None)):
    if not check_admin(ps_session): raise HTTPException(401)
    form = await request.json()
    ids = form.get("ids", [])
    ...
    conn.execute(f"DELETE FROM features WHERE id IN ({placeholders})", ids)
    conn.commit(); conn.close()
    return {"ok": True, "deleted": len(ids)}

@app.get("/admin/features")
async def admin_list_features(suburb: str = "", state: str = "", postcode: str = "",
                               limit: int = 5000, ps_session: str = Cookie(None)):
    if not check_admin(ps_session): raise HTTPException(401)
    conn = get_db()
    if postcode:
        rows = conn.execute("SELECT * FROM features WHERE postcode=? ORDER BY confidence DESC LIMIT ?", (postcode, limit)).fetchall()
    elif suburb and state:
        rows = conn.execute("SELECT * FROM features WHERE suburb=? AND state=? ORDER BY confidence DESC LIMIT ?", (suburb, state, limit)).fetchall()
    else:
        rows = conn.execute("SELECT * FROM features ORDER BY confidence DESC LIMIT ?", (limit,)).fetchall()
    conn.close()
    # Convert sqlite3.Row to plain dicts
    cols = rows[0].keys() if rows else []
    result = [{col: row[col] for col in cols} for row in rows]
...
async def review_page():
    p = Path("static/review.html")
    if p.exists(): return HTMLResponse(p.read_text())
    return HTMLResponse("<html><body><h1>Review tool not found</h1></body></html>")
